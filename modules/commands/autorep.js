...
module.exports.run = function({ api, event, args }) {
	...
	if (content.indexOf(`del`) == 0) {
		...
		return readFile(__dirname + "/data/autorep.json", "utf-8", (err, data) => {
			...
			writeFile(__dirname + "/data/autorep.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("☑️ Đã xóa autorep thành công!", threadID, messageID));
		});
	}
		...
		if (shortin == shortout) return api.sendMessage("⚠️ 2 input và output phải khác nhau nha", threadID, messageID);
		if (!shortin) return api.sendMessage("⚠️ Thiếu input", threadID, messageID);
		if (!shortout) return api.sendMessage("⚠️ Thiếu output", threadID, messageID);
		return readFile(__dirname + "/data/autorep.json", "utf-8", (err, data) => {
			if (err) throw err;
			var oldData = JSON.parse(data);
			if (!oldData.some(item => item.id == threadID)) {
				...
				return writeFile(__dirname + "/data/autorep.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("☑️ Tạo autorep thành công", threadID, messageID));
			}
			else {
				...
				if (getShort.shorts.some(item => item.in == shortin)) {
					...
					api.sendMessage("⚠️ Autorep đã tồn tại trong nhóm này", threadID, messageID);
