...
let send_chess = (o, chess, send, _ = o.handleReply || {}, sid = o.event.senderID, mid = o.event.messageID, uid = chess.turn() == 'b'?_.competitor_id: _.author || sid)=>send({
 body: `[ PLAY CHESS ON BOT ]\n────────────────────\n🔄 Lượt phía quân ${chess.turn() == 'b'?'đen': 'trắng'}\n👤 Player: ${name(uid)}\n────────────────────\n📝 Reply tọa độ quân cờ để tiếp tục`,
 mentions: [{
 id: uid,
 tag: ''+name(uid),
 }],
...
exports.run = o=> {
 ...
 if (!competitor_id) return o.api.sendMessage(`📝 Hãy tag ai đó để làm đối thủ của bạn`, o.event.threadID, o.event.messageID);
